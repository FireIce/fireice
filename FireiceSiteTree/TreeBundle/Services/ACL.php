...
class ACL
{
    ...
    public function __construct(EntityManager $em, $aclProvider, $securityContext)
    {
        ...
        if ($this->securityContext->getToken())
        {
            ...
            if ($this->current_user == 'anon.')
                $this->current_user = $this->em->getRepository('DialogsBundle:users')->findOneBy(array('type' => 'anonymous'));
        }
    }
    ...
    public function getAclNoRightsArray($id_user)
    {
        ...
        {
            if (!isset( $array[$val['id_node']] ))
                $array[$val['id_node']] = array();
            
            if (!isset( $array[$val['id_node']][$val['id_module']] ))
                $array[$val['id_node']][$val['id_module']] = $val['not_rights'];
        }
    ...
    {        
        ...
        $securityIdentity = new \Symfony\Component\Security\Acl\Domain\RoleSecurityIdentity('group_'.$user->getGroups());      
    ...
    {   	   
        ...
        {
            $module = new \fireice\FireiceSiteTree\Dialogs\DialogsBundle\Entity\module();	
            $module->setId($key);
    ...
    public function checkUserPermissions($id_node, $object, $user, $mask)
    {
        ...
        {                  
            ...
            {
                if (isset($this->acl_no_rights_array[$id_node][$object->getId()]))
                    $not_rights = intval($this->acl_no_rights_array[$id_node][$object->getId()]);
                else
                    $not_rights = 0;                
