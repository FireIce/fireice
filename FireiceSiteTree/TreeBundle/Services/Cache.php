...
class Cache
{
    ...
    public function __construct($em, $acl)
    {
    ...
    private function compileSiteTreeStructure()
    {
        ...
        {
            if (!isset($nodes[$val['node_id']]))
                $nodes[$val['node_id']] = array(
        ...
        {                   
            if (!isset($nodes[$value['node_id']]['plugins']))
                $nodes[$value['node_id']]['plugins'] = array();
        ...
        foreach ($nodes as $key=>&$node)
        {
            ...
            foreach ($path as $v)
            {
                if (isset($nodes[$v]['plugins']['fireice_node_name']['value']))
                    $name_path[] = $nodes[$v]['plugins']['fireice_node_name']['value'];   
                else
                    $name_path[] = $key;
            }
