...
class RightsModel
{
    ...
    {        
        ...
        {                    
            ...
            {
                $identy_group = new \Symfony\Component\Security\Acl\Domain\RoleSecurityIdentity('group_'.$val['groupid']);
            ...
            foreach ($groups_rights['group_'.$val['groupid']] as $right)
            {
                ...
                if (isset($users_not_rights[$val['id']]))
                    $not_rights = intval($users_not_rights[$val['id']]);
                else
                    $not_rights = 0;
    ...
    public function getUser($request)
    {
        ...
        $identy_group = new \Symfony\Component\Security\Acl\Domain\RoleSecurityIdentity('group_'.$user_result['groupid']);
        ...
        foreach ($group_rights as $right)
        {
            ...
            if (count($notrights_result) > 0)
                $not_rights = intval($notrights_result[0]['not_rights']);
            else
                $not_rights = 0;
    ...
    {                   
        ...
        {
            if ($val == '0')  
                $builder->add( $this->acl->getValueMask($key) );                        
        }
        ...
        if ($builder->get() != 0)
        {
            ...
            $aclnodesrights = new \fireice\FireiceSiteTree\Dialogs\DialogsBundle\Entity\aclnodesrights(); 
